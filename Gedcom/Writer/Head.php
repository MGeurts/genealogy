...
class Head
{
    ...
    public static function convert(\Gedcom\Record\Head &$head, $format = self::GEDCOM55)
    {
        ...
        $sour = $head->getSour();
        if ($sour) {
            $_convert = Head\Sour::convert($sour, $level);
            $output .= $_convert;
        }
        ...
        $date = $head->getDate();
        if ($date) {
            $_convert = Head\Date::convert($date, $level);
            $output .= $_convert;
        }
        ...
        $gedc = $head->getGedc();
        if ($gedc) {
            $_convert = Head\Gedc::convert($gedc, $level);
            $output .= $_convert;
        }
        ...
        $char = $head->getChar();
        if ($char) {
            $_convert = Head\Char::convert($char, $level);
            $output .= $_convert;
        }
        // PLAC
        $plac = $head->getPlac();
        if ($plac) {
            $_convert = Head\Plac::convert($plac, $level);
            $output .= $_convert;
        }
