<div class="flex flex-col rounded bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 text-neutral-800 dark:text-neutral-50"">
        ...
        <div class="flex flex-wrap gap-2 justify-center items-start">
            ...
            <div class="flex-grow min-w-max max-w-full flex-1 text-end">
                @if (auth()->user()->hasPermission('person:update'))
                    <a wire:navigate href="/people/{{ $person->id }}/edit-family">
                        <x-button.primary class="!p-2" title="{{ __('person.edit_family') }}">
    ...
    <div class="grid grid-cols-6">
        ...
        <div class="col-span-4 pr-2 py-2 border-b">
            @if ($person->parents)
                <x-link wire:navigate href="/people/{{ $person->parents->person_1->id }}" class="{{ $person->parents->person_1->isDeceased() ? '!text-danger' : '' }}">
                    <b>{{ $person->parents->person_1->name }}</b>
                </x-link>
                <x-icon.tabler icon="{{ $person->parents->person_1->sex == 'm' ? 'gender-male' : 'gender-female' }}" />
                <br />
                <x-link wire:navigate href="/people/{{ $person->parents->person_2->id }}" class="{{ $person->parents->person_2->isDeceased() ? '!text-danger' : '' }}">
                    <b>{{ $person->parents->person_2->name }}</b>
                </x-link>
        ...
        <div class="col-span-4 pr-2 py-2">
            @if ($person->currentPartner())
                <x-link wire:navigate href="/people/{{ $person->currentPartner()->id }}" class="{{ $person->currentPartner()->isDeceased() ? '!text-danger' : '' }}">
                    <b>{{ $person->currentPartner()->name }}</b>
                </x-link>
