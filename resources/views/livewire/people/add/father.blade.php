        ...
        <x-ts-tab.items tab="{{ __('person.add_new_person_as_father') }}">
            <div class="p-4 rounded-sm md:w-192 bg-neutral-200">
                ...
                <div class="grid grid-cols-6 gap-5">
                    ...
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-input wire:model="fatherForm.firstname" id="firstname" label="{{ __('person.firstname') }} :" wire:dirty.class="bg-yellow-200 dark:text-black"
                            autocomplete="firstname" />
                    </div>

                    {{-- surname --}}
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-input wire:model="fatherForm.surname" id="surname" label="{{ __('person.surname') }} : *" wire:dirty.class="bg-yellow-200 dark:text-black" autocomplete="surname" />
                    </div>

                    {{-- birthname --}}
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-input wire:model="fatherForm.birthname" id="birthname" label="{{ __('person.birthname') }} :" wire:dirty.class="bg-yellow-200 dark:text-black"
                            autocomplete="birthname" />
                    </div>

                    {{-- nickname --}}
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-input wire:model="fatherForm.nickname" id="nickname" label="{{ __('person.nickname') }} :" wire:dirty.class="bg-yellow-200 dark:text-black" autocomplete="nickname" />
                    </div>
                    ...
                    {{-- yob --}}
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-input wire:model="fatherForm.yob" id="yob" label="{{ __('person.yob') }} :" wire:dirty.class="bg-yellow-200 dark:text-black" autocomplete="yob" type="number" max="{{ date('Y') }}" />
                    </div>

                    {{-- dob --}}
                    <div class="col-span-6 md:col-span-3">
                        <x-ts-date wire:model="fatherForm.dob" id="dob" label="{{ __('person.dob') }} :" wire:dirty.class="bg-yellow-200 dark:text-black" format="YYYY-MM-DD" :max-date="now()"
                            placeholder="{{ __('app.select') }} ..." />
                    </div>

                    {{-- pob --}}
                    <div class="col-span-6">
                        <x-ts-input wire:model="fatherForm.pob" id="pob" label="{{ __('person.pob') }} :" wire:dirty.class="bg-yellow-200 dark:text-black" autocomplete="pod" />
                    </div>
                    <x-hr.narrow class="col-span-6 my-0!" />
            ...
            <div class="flex items-center justify-end pt-4 text-right">
                ...
                <div class="flex-1 grow max-w-full text-end">
                    <x-ts-button color="secondary" class="mr-1" wire:click="resetFather()" wire:dirty>
                        {{ __('app.cancel') }}
                    </x-ts-button>
                    ...
                    <x-ts-select.styled wire:model="fatherForm.person_id" id="person_id" label="{{ __('person.person') }} : *" :options="$persons" select="label:name|value:id"
                        ...
                        <x-slot:after>
                            <div class="w-full px-2 mb-2">
                                <x-ts-alert title="{{ __('app.nothing_available') }}" text="{{ __('person.use_tab') . ' : ' . __('person.add_new_person_as_father') }}" color="cyan" />
                            </div>
                        </x-slot:after>
                    </x-ts-select.styled>
                </div>
