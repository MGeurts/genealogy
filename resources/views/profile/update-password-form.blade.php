<x-form-section submit="updatePassword">
    <x-slot name="title">
        <div class="dark:text-gray-400">{{ __('user.update_password') }}</div>
    </x-slot>

    <x-slot name="description">
        <div class="dark:text-gray-100">{{ __('user.update_password_secure') }}</div>
    </x-slot>
    ...
    <x-slot name="actions">
        <x-action-message class="mr-3 rounded-sm bg-emerald-200 p-3 text-xs text-emerald-600" role="alert" on="saved">
            {{ __('app.saved') }}
        </x-action-message>
