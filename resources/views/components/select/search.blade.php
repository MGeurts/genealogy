...
<div x-data="AlpineSelect({
    ...
    limit: {{ $limit }},
})" x-init="init()" @click.away="closeSelect()" @keydown.escape="closeSelect()" @keydown.arrow-down.prevent="increaseIndex()" @keydown.arrow-up.prevent="decreaseIndex()"
    @keydown.enter="selectOption(Object.keys(options)[currentIndex])">

    <div class="relative content-center w-full p-1 text-left bg-white border rounded sm:text-sm sm:leading-5"
        x-bind:class="{ 'border-blue-300 ring ring-blue-200 ring-opacity-50': open, 'bg-gray-200 cursor-default': disabled }" @click.prevent="toggleSelect()">
        <div id="placeholder">
            ...
            <div class="flex flex-wrap space-x-1" x-cloak x-show="selected.length > 0">
                <template x-for="(key, index) in selected" :key="index">
                    <div class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                        ...
                        <div x-show="!disabled" x-bind:class="{ 'cursor-pointer': !disabled }" class="w-4" @click.prevent.stop="deselectOption(index)"><svg xmlns="http://www.w3.org/2000/svg"
        ...
        @else
            <div class="flex flex-wrap" x-cloak x-show="selected">
                <div class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                    ...
                    <div x-show="!disabled" x-bind:class="{ 'cursor-pointer': !disabled }" class="h-4" @click.prevent.stop="deselectOption()"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
            ...
            <div class="mt-0.5 w-full bg-white border-gray-300 rounded-b-md border absolute top-full left-0 z-30" x-show="open" x-cloak>

                <div class="relative z-30 w-full p-2 bg-white">
                    <input type="search" x-model="search" x-on:click.prevent.stop="open=true"
                        class="block w-full p-2 border border-gray-300 rounded focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 sm:text-sm sm:leading-5">
                ...
                <div x-ref="dropdown" class="relative z-30 p-2 overflow-y-auto max-h-60">
                    ...
                    <template x-for="(key, index) in Object.keys(options)" :key="index">
                        @isset($attributes['multiple'])
                            <div class="px-2 py-1"
                                x-bind:class="{
                                    'bg-gray-300 text-white hover:none': selected.includes(key),
                        ...
                        @else
                            <div class="px-2 py-1"
                                x-bind:class="{
                                    ...
                                    'hover:bg-blue-500 hover:text-white cursor-pointer': !(selected ==
                                        key),
                                    'bg-blue-500 text-white': currentIndex == index
                                }"
                                @click.prevent.stop="selectOption(key)" x-text="Object.values(options)[index]">
