<div>
    {{-- search box section --}}
    <div class="p-2 sticky top-[6.5rem] z-20 bg-gray-100 dark:bg-gray-900" wire:key="search-box-section">
        ...
        'scope' => auth()->user()->is_developer ? strtoupper(e(__('team.all_teams'))) : e(auth()->user()->currentTeam->name),
            ...
            <div class="flex flex-wrap items-start gap-2">
                ...
                <div class="flex-1 grow w-full" x-init="$nextTick(() => { $el.querySelector('input')?.focus() })">
                    <x-ts-input wire:model.live.debounce.500ms="search" icon="tabler.search" hint="{{ __('app.people_search_tip') }}" placeholder="{{ __('app.people_search_placeholder') }}" clearable />
                </div>
    ...
    @if (count($people) > 0)
        ...
        <div class="p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5">
            @foreach ($people as $person)
                <livewire:people::person :person="$person" :key="'person-' . $person->id . '-' . md5($search ?? '') . '-' . $people->currentPage()" />
            @endforeach
        </div>
    @else
        {{-- carousel --}}
        <div class="p-5 mx-auto text-center max-w-6xl" wire:key="carousel-section">
